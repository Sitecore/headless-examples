...
/**
 * MULTISITE-UPDATE
 * config.js now exports a function to generate a config, instead of a full config
 */
const getConfig = (appName, siteName) => {
  ...
  /**
   * MULTISITE-UPDATE
   * Use siteName with dictionary service
   */
  ...
  const config = {
    ...
    /**
     * MULTISITE-UPDATE
     * Ensure the site name is sent to the Layout Service via query string.
     */
    ...
    pathRewriteExcludeRoutes: [
      ...
      (process.env.SITECORE_PATH_REWRITE_EXCLUDE_ROUTES || "").split("|")
    ),
    /**
     ...
     */
    debug:
      (process.env.SITECORE_ENABLE_DEBUG || "").toLowerCase() === "true" ||
      false,
    /**
    ...
    proxyOptions: {
      ...
      /**
       * MULTISITE-UPDATE
       * Disabled xfwd since it was breaking the proxy and CM behind traefik
       */
      xfwd: false
    },
    /**
