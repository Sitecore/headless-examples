...
Write-Host "Installing node_modules..." -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
    Write-Error "npm install failed, see errors above."
}
...
dotnet sitecore login --cm https://cm.nextjsandazureb2c.localhost/ --auth https://id.nextjsandazureb2c.localhost/ --allow-write true
if ($LASTEXITCODE -ne 0) {
...
Invoke-RestMethod "https://cm.nextjsandazureb2c.localhost/sitecore/admin/PopulateManagedSchema.aspx?indexes=all" -Headers @{Authorization = "Bearer $token"} -UseBasicParsing | Out-Null
...
Start-Process https://cm.nextjsandazureb2c.localhost/sitecore/
Start-Process https://www.nextjsandazureb2c.localhost/
